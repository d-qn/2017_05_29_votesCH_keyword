...
Since 1848, Swiss had their say over 600 nationwide ballots. swissinfo.ch looked


Nearly half of the states in the world have some form of direct legislation. But Switzerland stands alone in how extensively it uses it. In most part of the world, referendums and initiatives remain rare. But Swiss go to polls for national votes about four times a year. To give their verdict on? swissinfo.ch looked at the ballot themes since 184 
...
Switzerland‟s first constitution of 1848 followed the principle of representation. It contained only two direct-democratic rights: the mandatory constitutional referendum and the popular ini- tiative for a complete revision of the federal constitution. The other popular rights were introduced gradually in a process which took many years. We have to consider them as concessions which the political elite had to make to opposition forces.
...
html_wrap <- function(x, width = 60, linebreak = "<br>") {
...
singlePlot <- function(ddd, kw, lang = 'EN') {
  ...
  dddd$tp <- paste0(
    ...
    '</td></tr>',
		...
		ifelse(dddd$Result == "no", txt["tp.refused",lang], txt["tp.accepted",lang]),
		'</td></tr>','</table>')
  ...
  hc3 <- hc2 %>% hc_tooltip(
    ...
    useHTML = T, borderWidth = 3, 
    backgroundColor = 'rgba(255,255,255,1)', style = list(padding = 3)
  ) %>%
    hc_title(
  ...
  hc3 %>%
    hc_subtitle(
      floating = T, useHTML = T, text = txt[paste0(kw, ".sub"), lang], 
      x = 2, y = 45, widthAdjust = -110,
...
wc <- d3wordcloud(names(kwf), kwf, colors=sample(swi_col, length(kwf), T) , tooltip = T, 
...
lang <- 'EN'
  ...
  save_html(
    tags$html(
      ...
      tags$body(    
        h2(HTML("What are the most common themes in Switzerland's federal ballot?")),
        # div(class = "descr", HTML("The most common words present in watch brands' press releases for Baselworld 2017. 137 press releases were analysed, only the words encountered more than 20 times are shown below. Words are sized according to their frequency. Hover over a term to see its count.")),
        div(class="graphic", wc),
        div(id = "cite", HTML(source)),
        HTML(iframeresizer)
      )), file = paste0("keyword_worldcloud_", lang, ".html"), libdir = "js")  

...
singlePlot <- function(ddd, kw, lang = 'EN') {
  ...
  if(!is.na(txt[paste0(kw, ".sub"), lang])) {
    hc3 <- hc3 %>% hc_subtitle(
      ...
      style = list("fontSize" = ifelse(lang == "AR", "0.95em", "1.1em"), "color" = "#0d0d0d")
    )  
  } 
  hc3
}
...
wc <- d3wordcloud(names(kwf), kwf, 
                  colors = rev(colorRampPalette(c(swi_col[c(2,7,3,6,4,5)], '#f7f5ed'))(length(kwf))),
                  #colors = sample(c(swi_col[-c(1, length(swi_col))]), length(kwf), T) , 
                  tooltip = T, padding = 1,
                  spiral = "rectangular", 
                  rotate.min = -30, rotate.max = -30, 
                  rangesizefont = c(10, 35),
